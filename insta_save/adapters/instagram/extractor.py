...
def _extract_author(page) -> str | None:
    """The IG handle (username), taken from the profile-link href (/handle/).
    ...
    for link in page.locator(ROLE_LINK_SEL).all():
        ...
        if not _USERNAME_RE.match(href) or href in _NON_USER_HREFS:
            continue
...
def extract_metadata_ytdlp(url: str, cookies_txt: str) -> dict:
    ...
    base["author"] = _author_from_ytdlp_meta(meta)
